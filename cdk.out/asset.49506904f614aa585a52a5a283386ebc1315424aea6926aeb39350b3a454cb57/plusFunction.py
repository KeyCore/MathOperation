...
def lambda_handler(event, context):
    ...
    # First, attempt to look up a prerecorded result of operand1+operand2 (ADD#operand1, operand2, result)
    ...
    if operand2>0:
        # If operand2 is positive, look up the highest addend recorded as being added to operand1, in the table
        # (ADD#operand1, highAddend, result)
        rs=table.query(KeyConditionExpression=Key('operation').eq(operation),ScanIndexForward=False, Limit=1)    

        highAddend=rs['Items'][0]['operand2']
        mathResult=rs['Items'][0]['result']
        # Fill the gap between highAddend and operand2, by inserting items in the table
        for i in range (int(highAddend)+1, int(operand2)+1):
        ...
        # If operand2 is negative, look up the lowest addend recorded as being added to operand1, in the table
        # (ADD#operand1, lowAddend, result)
        rs=table.query(KeyConditionExpression=Key('operation').eq(operation),ScanIndexForward=True, Limit=1)    
        lowAddend=rs['Items'][0]['operand2']
        mathResult=rs['Items'][0]['result']
        # Fill the gap between lowAddend and operand2, by inserting items in the table
        for i in range (int(lowAddend)-1, int(operand2)-1, -1):
            mathResult=mathResult-1
            rs=table.put_item(
                Item={
                    'operation': operation,
